...
export const Order = ()=>{
	...
	[ errors, setErrors ] = useState([]); // バリデーション・エラーチェック用の配列
    ...
    useEffect(
        () => {
            ...
            if (cartData.length !== 0 && ramenData.length !== 0) {
                ...
                const cartIdList = carts.map(cart => cart.id) // cart内のidだけのリストを作成

                let matchRamen = cartIdList.map( // findした値の配列を生成する！ => matchラーメン配列
                    cartid => {
                ...
                carts.forEach(cart => {
                    ...
                    let Match = matchRamen.find(ramen => ramen.id === cart.id) // idが一致するものを一つ格納！
		...
		.then(snapshot => {
				snapshot.forEach(doc => {
					if (doc.data().status === 0) {
						newCart.push({ ...doc.data(), id: doc.id }) // status0のカート情報をpushして、idも付与する。
					}
				})
			},
